...
const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
        ...
        // Handle case where benchmark might be null (e.g. for 1M)
        const benchmark = payload.length > 1 ? payload[1].value : null;
        const alpha = benchmark !== null ? (fund - benchmark).toFixed(2) : null;
                ...
                <div className="space-y-2">
                    ...
                    {benchmark !== null && (
                        <div className="flex items-center justify-between gap-8">
                            ...
                            <span className="font-mono font-semibold text-slate-600">{benchmark.toFixed(2)}%</span>
                        </div>
                    )}
