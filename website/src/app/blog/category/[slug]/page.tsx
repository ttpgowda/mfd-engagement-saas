...
export default async function CategoryPage({ params }: PageProps) {
    ...
    return (
        <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <div className="mb-12">
                <Link href="/blog" className="text-sm font-semibold leading-6 text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 mb-4 inline-flex items-center gap-1">
                    <ArrowLeft className="h-4 w-4" /> All Articles
                </Link>
                <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100 sm:text-4xl capitalize">
                    {category}
                </h1>
                ...
                {posts.map((post) => (
                    <article key={post.slug} className="flex flex-col items-start bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 transition-shadow hover:shadow-md">
                        <div className="flex items-center gap-x-4 text-xs w-full mb-4">
                            ...
                            <span className="relative z-10 rounded-full bg-indigo-50 dark:bg-indigo-900/30 px-3 py-1.5 font-medium text-indigo-700 dark:text-indigo-300">
                                {post.category}
                            </span>
                        </div>
