// Retirement Preparedness Survey (Goal: Retirement)
export const RETIREMENT_PREP_SURVEY = {
    meta: {
    ...
    questions: [
...

        {
            id: 6,
            question:
                "Do you have any employer/guaranteed pension, provident fund, or annuity income expected in retirement?",
            type: "single-select",
...

        {
            id: 7,
            question:
                "Do you have health insurance that will reasonably cover major medical costs during retirement (or plan to have one)?",
            type: "single-select",
...

        {
            id: 9,
            question:
                "What proportion of your retirement portfolio is currently in growth assets (equity / equity funds) vs debt?",
            type: "single-select",
            options: [
                "Select",
    ...
    // Follow-ups — show when borderline or contradictions exist (e.g., short horizon + low corpus)
    followUps: [
        {
            id: 21,
            question:
                "If your corpus falls short at retirement, which fallback would you prefer (choose one)?",
            type: "single-select",
            options: [
                "Select",
        ...
        },
        {
            id: 22,
            question:
                "Do you have a written retirement plan (with target corpus, contributions, projected returns, withdrawal rules)?",
            type: "single-select",
        ...
        },
        {
            id: 23,
            question:
                "Have you planned for long-term care or eldercare costs (separate from health insurance)?",
            type: "single-select",
    ...
    scoring: {
        explanation:
            "Map each answer to its 0–4 score, multiply by the question weight, sum across answered questions, and normalise to a 0–100 scale.",
        formula:
            `normalized_score = round( (sum(question_score * question_weight) / sum(max_score_per_answered_question * question_weight)) * 100 )`,
        // use max_score_per_answered_question = 4 for questions that use 0-4 scoring
        thresholds: {
        ...
        // Example mappings for quick UI messages
        recommendedActions: {
            notPrepared: [
    ...
    uiHints: {
        recommendedOrder: [
            "1: Retirement age (Q1) + desired income (Q2)",
            "2: Years to retirement (Q3) + current corpus (Q4)",
            "3: Contribution rate (Q5) + employer pension (Q6)",
            "4: Protection (health insurance Q7) + debt (Q8)",
            "5: Allocation + behaviour (Q9 - Q13)",
            "6: Follow-ups if score is Partially Prepared or Not Prepared, or contradictions exist"
        ],
        progressiveDisclosure:
            "Keep first 6–8 questions as core to reduce friction. Only ask follow-ups when score is borderline or answers contradict (e.g., short horizon but low corpus).",
        tips:
            "Provide an inline 'estimate my corpus' helper that asks current age, retirement age, desired annual income, and inflation/return assumptions — use it when user selects 'Not sure' for corpus or target income.",
        showWhen:
            "If emergency fund = No or health insurance = No, show an in-app banner recommending protection steps before increasing equity exposure."
    }
};
